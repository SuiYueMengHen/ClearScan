...
Java_com_arm_aichat_internal_InferenceEngineImpl_init(JNIEnv *env, jobject /*unused*/, jstring nativeLibDir) {
    ...
    // The CPU backend is linked into libai-chat.so. Dynamic backend loading is
    // unreliable on some vendor linkers, including HarmonyOS Android runtime.
    (void) env;
    (void) nativeLibDir;
    llama_backend_init();
    LOGi("Static backend initialized; registered backends=%zu", ggml_backend_reg_count());
}
...
Java_com_arm_aichat_internal_InferenceEngineImpl_load(JNIEnv *env, jobject, jstring jmodel_path) {
    if (ggml_backend_reg_count() == 0) {
        LOGe("Cannot load model: no GGML backend is registered");
        return 10;
    }
...
extern "C"
JNIEXPORT jint JNICALL
Java_com_arm_aichat_internal_InferenceEngineImpl_backendCount(JNIEnv *, jobject) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_arm_aichat_internal_InferenceEngineImpl_reset(JNIEnv *, jobject) {
